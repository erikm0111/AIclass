...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        value = 0
        for (nextSt, Tprobability) in self.mdp.getTransitionStatesAndProbs(state, action):
            reward = self.mdp.getReward(state, action, nextSt)
            value += Tprobability * (reward + self.discount*self.values[nextSt])
        return value
    ...
    def computeActionFromValues(self, state):
        ...
        for possact in self.mdp.getPossibleActions(state):
            actions[possact] = self.computeQValueFromValues(state, possact)
